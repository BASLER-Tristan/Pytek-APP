...
def pipeline(data,
    ...
    linearized_date = [item for item in linearized_date for _ in range(len(data.columns))]
...
def fit_model(model,
              res,
              date_limit):
    X = res[res.date < date_limit][['year', 'month',
                                    'M -12',
                                    'M -11',
                                    'M -10',
                                    'M -9',
...
def result_pipeline(model,
    ...
    ### Overlap data
    y_pred = model.predict(rep[['year', 'month',
                                'M -12',
                                'M -11',
                                'M -10',
                                'M -9',
                                ...
                                'M -2',
                                'M -1', ]])
    date_scale = pd.to_datetime(rep['date']) + pd.DateOffset(months=1)
    ...
    for product in list(data.columns):
        past_prediction[product] = result_on_training[result_on_training[2].str.startswith(product)].reset_index()[0]
    past_prediction.index = date_scale.unique()

    ### metrics
    data_past = data[data.index <= date]
    dffm = (data_past - res) ** 2
    ...
    dff = pd.concat([
        past_prediction,
        data_past[data_past.index > past_prediction.index.max()]
    ])
    dff.columns = [col + '_xgboost' for col in dff.columns]
    dffm.columns = [col + '"_error_xgboost"' for col in dffm.columns]
